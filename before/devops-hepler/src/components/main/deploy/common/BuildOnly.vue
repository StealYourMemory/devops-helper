  ...
  <div>
    <el-form
      ...
      <div style="display: flex; justify-content: flex-end; margin-right: 10px;margin-top: 40px">
        <el-button size="large" @click="doBuild(ruleFormRef)" :loading="doBuildLoading">
          <el-space size="small">
...
import {buildOnly} from "@/apis/commonDeployApi";
...
import {BUILD_ONLY_REQUEST_KEY, BuildOnlyName} from "@/utils/Constants";
...
async function doBuild(formEl: FormInstance | undefined) {
  if (!formEl) return
  await formEl.validate(async (valid, fields) => {
    if (valid) {
      ...
      await sleep(500);
      const result = await buildOnly(deployRequest.value);
      doBuildLoading.value = false;
      if (result.success) {
        LocalStorageUtil.set(BUILD_ONLY_REQUEST_KEY, result.data);
        ElMessage.success("请求打包成功");
      } else {
        ElMessage.error(result.msg);
      }
    } else {
